...
import { Environment, LogDashboardModel, TablePaginationModel } from 'common/types';
...
export default function Logs(props:{
  ...
  function refreshTable(){
    const url = `${props.env.apiBase}Dashboard/GetLogs?page=${filter.page}&row=${filter.row}&operation=${filter.operation}&startDate=${filter.startDate}&endDate=${filter.endDate}`;
    Axios.get<TablePaginationModel<LogDashboardModel>>(url)
      .then((response) => {
        ...
        <Table striped bordered hover variant="dark" style={{tableLayout:'auto', borderCollapse:'collapse', width:'100%' }}>
          ...
          <tbody>
            {tableModel.records.map((log, index) => {
              const badgeType = log.operation === 'D' ? 'danger' : log.operation === 'U' ? 'success' : 'primary';
              return (
                <tr key={index}>
