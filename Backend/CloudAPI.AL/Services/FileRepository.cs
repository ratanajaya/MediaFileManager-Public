...
public class FileRepository
{
    ...
    public FileRepository(ConfigurationModel config, IAlbumInfoProvider ai, ISystemIOAbstraction io, IDbContext db, LibraryRepository lib, ILogger logger, MediaProcessor media) {
        _config = config;
        _ai = ai;
        _io = io;
        _db = db;
    ...
    public (FileInfoModel fi, int count) GetRandomCoverPageInfoByQuery(string query) {
        ...
        FileInfoModel GetPageInfoByPath(string albumPath, int pageIndex) {
            ...
            return new FileInfoModel {
                ...
                UncPathEncoded = Path.GetRelativePath(_config.LibraryPath, targetFullPath),
            };
        }
        #endregion
        ...
        try {
            ...
            try {
                ...
                var index = PrimitiveHelper.GenerateNumberFromSeed(seed, count);
    ...
    public void UpdateAlbumChapter(ChapterUpdateParamModel param) {
        try {
            ...
            _lib.SaveAlbumMetadata(targetAlbum);
            _db.SaveChanges();
        }
        catch(Exception e) {
            _logger.Error($"FileRepository.DeleteFile{Environment.NewLine}" +
